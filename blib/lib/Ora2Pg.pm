...
sub export_schema
{
	...
	# Save checksums if complex type validation is enabled
	if ($self->{checksum_validator}->{enabled} && keys %{$self->{checksum_validator}->{checksums}}) {
		$self->logit("Saving complex type checksums to $self->{checksum_validator}->{checksum_file}...\n", 1);
		$self->{checksum_validator}->save_checksums();
		$self->logit("Saved checksums for $self->{checksum_validator}->{tables_processed} tables and $self->{checksum_validator}->{rows_validated} rows\n", 1);
	}
...
sub _init
{
	...
	# Initialize export log file (overwrite mode)
	my $export_log = "ora2pg_export.log";
	if ($self->{output_dir}) {
		$export_log = $self->{output_dir} . "/" . $export_log;
	}
	if (open(my $fh, '>', $export_log)) {
		$self->{export_log_fh} = $fh;
		# Enable autoflush
		my $old_fh = select($fh);
		$| = 1;
		select($old_fh);
	} else {
		warn "Could not open log file $export_log: $!";
	}
        ...
        foreach my $t (@{$self->{export_type}})
	{
		...
		elsif ($self->{type} eq 'TEST_COMPLEX_TYPES')
		{
			# Complex type checksum validation
			$self->_complex_type_validation();
			$self->{dbh}->disconnect() if ($self->{dbh});
			exit 0;
		}
		elsif ($self->{type} eq 'TEST_COMPLEX_TYPES')
		{
			...
			# Validate complex type checksums
			$self->_complex_type_validation();
			$self->{dbhdest}->disconnect() if ($self->{dbhdest});
			$self->{dbh}->disconnect() if ($self->{dbh});
			exit 0;
	...
	# Initialize checksum validator for complex types
	$self->{checksum_validator} = Ora2Pg::ChecksumValidator->new(
		enabled              => $self->{complex_checksum_validation} || 0,
		sample_size          => $self->{complex_checksum_sample_size} || 1000,
		sample_percentage    => $self->{complex_checksum_sample_percentage} || 5,
		hash_algorithm       => $self->{complex_checksum_algorithm} || 'SHA256',
		validation_output    => $self->{complex_checksum_validation_output} || 'complex_validation_report.txt',
		checksum_file        => $self->{complex_checksum_file} || 'complex_checksums.json',
	);
		...
		} keys %{$self->{tables}})
	{
		...
		if (exists $self->{tables}{$table}{column_info})
		{
				...
				} keys %{$self->{tables}{$table}{column_info}})
			{
				...
				# Check for complex types (Oracle only)
				if (!$self->{is_mysql} && !$self->{is_mssql}) {
					my $current_type = uc($f->[1]);
					# Remove size/precision for check
					$current_type =~ s/\(\d+(?:,\d+)?\)//;
					...
					if (!exists $Ora2Pg::Oracle::SQL_TYPE{$current_type} &&
						...
						$current_type !~ /^XMLTYPE/ &&
						$current_type !~ /^SDO_GEOMETRY/ &&
						$current_type !~ /^ST_GEOMETRY/
					   ) {
						$self->log_complex_type_warning($table, $f->[0], $f->[1]);
					}
				}
		...
		for my $k (0 .. $#{$name})
		{
			...
			if (!$self->{is_mysql} && !$self->{is_mssql}) {
				...
				if (!exists $Ora2Pg::Oracle::SQL_TYPE{$current_type} &&
					...
					$self->log_complex_type_warning($table, $realcolname, $src_type->[$k]);
				}
			}
...
sub format_data
{
	...
	foreach my $row (@$rows) {
		...
		# Generate checksum for complex types if validation is enabled
		if ($self->{checksum_validator}->{enabled}) {
			# Check if this row contains complex types
			my $has_complex_types = 0;
			for (my $i = 0; $i < @$src_data_types; $i++) {
				if ($self->{checksum_validator}->is_complex_type($src_data_types->[$i])) {
					$has_complex_types = 1;
					last;
				}
			}
			
			# If this row has complex types and should be sampled
			if ($has_complex_types && $self->{checksum_validator}->should_sample_row($row_number, scalar(@$rows))) {
				# Determine primary key columns for this table to use as row identifier
				my @pk_columns = ();
				my @pk_indices = ();
				
				# Find primary key constraint for this table
				if (exists $self->{tables}{$table}{unique_key}) {
					foreach my $consname (keys %{$self->{tables}{$table}{unique_key}}) {
						if ($self->{tables}{$table}{unique_key}->{$consname}{type} eq 'P') {
							# Found primary key - get column names
							@pk_columns = @{$self->{tables}{$table}{unique_key}->{$consname}{columns}};
							
							# Map column names to indices in the row data
							for my $pk_col (@pk_columns) {
								for (my $i = 0; $i < @$fields; $i++) {
									if (uc($fields->[$i]) eq uc($pk_col)) {
										push @pk_indices, $i;
										last;
									}
								}
							}
							last; # Only one primary key per table
						}
					}
				}
				...
				if (@pk_indices) {
					...
					$row_id_value = join('_', map { defined $row->[$_] ? $row->[$_] : 'NULL' } @pk_indices);
				} elsif (defined $row->[0]) {
				...
				my $row_id = "${table}_${row_id_value}";
				
				# Generate checksum before formatting
				my $checksum = $self->{checksum_validator}->generate_row_checksum($row, $data_types, $src_data_types, $table, $row_id);
				$self->{checksum_validator}->store_checksum($table, $row_id, $checksum, 'export');
				
				$self->{checksum_validator}->{rows_validated}++;
			}
		}
	...
	if ($self->{checksum_validator}->{enabled}) {
		$self->{checksum_validator}->{tables_processed}++;
	}
...
sub logit
{
	...
	if ($self->{debug} >= $level) {
		if (defined $self->{export_log_fh}) {
			$self->{export_log_fh}->print($message);
		}
	...
	if ($critical)
	{
		...
		$self->{export_log_fh}->close() if (defined $self->{export_log_fh});
		$self->{dbh}->disconnect() if ($self->{dbh});
		$self->{dbhdest}->disconnect() if ($self->{dbhdest});
		die "Aborting export...\n";
	}
...
sub log_complex_type_warning
{
	my ($self, $table, $column, $type) = @_;

	my $msg = "[WARNING] [COMPLEX_TYPE] Table: $table, Column: $column, Type: $type - Manual review required\n";
...
=head2 _complex_type_validation

This function validates checksums for complex Oracle data types between
source and target PostgreSQL database to ensure data integrity during migration.

=cut

sub _complex_type_validation
{
	my $self = shift;
	
	$self->logit("Complex type checksum validation between Oracle and PostgreSQL...\n", 1);
	
	# Initialize checksum validator if not already done
	if (!$self->{checksum_validator}) {
		$self->{checksum_validator} = Ora2Pg::ChecksumValidator->new(
			enabled              => $self->{complex_checksum_validation} || 0,
			sample_size          => $self->{complex_checksum_sample_size} || 1000,
			sample_percentage    => $self->{complex_checksum_sample_percentage} || 5,
	...
	# Enable checksum validation if not already enabled
	if (!$self->{checksum_validator}->{enabled}) {
		$self->{checksum_validator}->{enabled} = 1;
		$self->logit("Enabling checksum validation for complex type validation...\n", 1);
	}
	
	# Load existing checksums from export phase
	my $checksum_file = $self->{checksum_validator}->{checksum_file};
	if (!$checksum_file || $checksum_file eq '') {
		# Try common checksum file locations
		my @possible_files = (
			'complex_checksums.json',
			'test_checksums.json',
			'checksums.json'
		);
		...
		foreach my $file (@possible_files) {
			if (-f $file) {
				$checksum_file = $file;
				$self->{checksum_validator}->{checksum_file} = $file;
				last;
			}
		}
	}
	
	if ($checksum_file && -f $checksum_file && $self->{checksum_validator}) {
		print STDERR "DEBUG: Found checksum file $checksum_file, proceeding with checksum validation\n";
		$self->{checksum_validator}->load_checksums();
		$self->logit("Loaded existing checksums from $checksum_file\n", 1);
		
		# Establish PostgreSQL connection for validation
		if (!$self->{dbhdest}) {
			$self->{dbhdest} = $self->_send_to_pgdb();
			if (!$self->{dbhdest}) {
				$self->logit("ERROR: Cannot connect to PostgreSQL for validation\n", 1);
				$self->logit("       Make sure PG_DSN, PG_USER, and PG_PWD are properly configured\n", 1);
				return;
			}
		}
	} else {
		print STDERR "DEBUG: Checksum validation conditions not met:\n";
		print STDERR "DEBUG: checksum_file = " . ($checksum_file || 'undef') . "\n";
		print STDERR "DEBUG: file exists = " . ($checksum_file && -f $checksum_file ? 'yes' : 'no') . "\n";
		print STDERR "DEBUG: validator exists = " . ($self->{checksum_validator} ? 'yes' : 'no') . "\n";
		$self->logit("INFO: No checksum file found for validation.\n", 1);
		$self->logit("      To enable checksum validation:\n", 1);
		$self->logit("      1. Set COMPLEX_CHECKSUM_VALIDATION=1 in config\n", 1);
		$self->logit("      2. Run: ora2pg -t COPY -c <config_file>\n", 1);
		$self->logit("      3. Then run: ora2pg -t TEST_COMPLEX_TYPES -c <config_file>\n", 1);
		$self->logit("\n", 1);
		$self->logit("Performing basic complex type validation without checksums...\n", 1);
		
		# Perform basic validation without checksums
		$self->_perform_basic_complex_type_validation();
		return;
	}
	
	my $validation_errors = 0;
	my $total_tables_validated = 0;
	my $total_rows_validated = 0;
	
	# Validate each table that has complex types and checksums
	foreach my $table_name (keys %{$self->{checksum_validator}->{checksums}}) {
		$self->logit("Validating complex types in table: $table_name\n", 1);
		
		# Query PostgreSQL to get the same rows and validate their checksums
		my $schema_prefix = '';
		if ($self->{pg_schema}) {
			$schema_prefix = $self->quote_object_name($self->{pg_schema}) . '.';
		} elsif ($self->{schema} && $self->{schema} ne 'public') {
			$schema_prefix = $self->quote_object_name(lc($self->{schema})) . '.';
		}
		
		my $sql = "SELECT * FROM " . $schema_prefix . $self->quote_object_name($table_name);
		if ($self->{where}{lc($table_name)}) {
			$sql .= " WHERE " . $self->{where}{lc($table_name)};
		}
		
		$self->logit("Executing validation query: $sql\n", 2) if ($self->{debug});
		
		my $sth = $self->{dbhdest}->prepare($sql);
		if (!$sth) {
			$self->logit("ERROR: Cannot prepare query for table $table_name: " . $self->{dbhdest}->errstr . "\n", 1);
			$validation_errors++;
			next;
		}
		
		if (!$sth->execute()) {
			$self->logit("ERROR: Cannot execute query for table $table_name: " . $self->{dbhdest}->errstr . "\n", 1);
			$validation_errors++;
			next;
		}
		
		# Get column information to identify data types
		my @data_types = @{$sth->{pg_type}} if $sth->{pg_type};
		my @src_types = (); 
		
		# Get PostgreSQL column type information for complex type detection
		my $schema_name = $self->{pg_schema} || lc($self->{schema}) || 'public';
		...
		my $col_sth = $self->{dbhdest}->prepare($col_sql);
		if ($col_sth && $col_sth->execute($schema_name, $table_name)) {
			while (my ($col_name, $data_type) = $col_sth->fetchrow_array()) {
				# Map PostgreSQL types back to source types for complex type detection
				if ($data_type =~ /\[\]$/) {
					# Array types: integer[] -> VARRAY
					my $base_type = $data_type;
					$base_type =~ s/\[\]$//;
					push @src_types, "${base_type}_ARRAY";
		...
		while (my $row = $sth->fetchrow_arrayref()) {
			$row_number++;
			my $row_id = "${table_name}_${row_number}";
			
			# Check if we have a checksum for this row
			if (exists $self->{checksum_validator}->{checksums}{$table_name}{$row_id}) {
				# Generate checksum for the imported data
				my $import_checksum = $self->{checksum_validator}->generate_row_checksum($row, \@data_types, \@src_types, $table_name, $row_id);
				
				# Store the import checksum
				$self->{checksum_validator}->store_checksum($table_name, $row_id, $import_checksum, 'import');
		...
		# Validate checksums for this table with live PostgreSQL data
		my $result = $self->{checksum_validator}->validate_checksums($table_name, $self->{dbhdest}, $schema_name);
		if ($result->{mismatches} > 0 || @{$result->{errors}}) {
			$validation_errors += $result->{mismatches} + scalar(@{$result->{errors}});
			$self->logit("VALIDATION ERRORS in table $table_name:\n", 1);
			foreach my $error (@{$result->{errors}}) {
				$self->logit("  - $error\n", 1);
			}
		} else {
			$self->logit("✓ All checksums validated successfully for table $table_name\n", 1);
		}
	}
	
	# Generate final validation report
	my $report_summary = $self->{checksum_validator}->generate_validation_report();
	
	# Log summary
	$self->logit("\n" . "="x60 . "\n", 1);
	$self->logit("COMPLEX TYPE VALIDATION SUMMARY\n", 1);
	$self->logit("="x60 . "\n", 1);
	$self->logit("Tables validated: $total_tables_validated\n", 1);
	$self->logit("Rows validated: $report_summary->{total_checked}\n", 1);
	$self->logit("Checksum mismatches: $report_summary->{total_mismatches}\n", 1);
	$self->logit("Total errors: $report_summary->{total_errors}\n", 1);
	
	if ($report_summary->{total_mismatches} == 0 && $report_summary->{total_errors} == 0) {
		$self->logit("✓ ALL COMPLEX TYPE VALIDATIONS PASSED - Data integrity verified!\n", 1);
	} else {
		$self->logit("✗ VALIDATION FAILURES DETECTED - See report for details\n", 1);
		$self->logit("Report saved to: $self->{checksum_validator}->{validation_output}\n", 1);
	}
	
	# Save updated checksums (now includes import phase)
	$self->{checksum_validator}->save_checksums();
	
	return $validation_errors;
}

=head2 _perform_basic_complex_type_validation

Performs basic complex type validation without requiring checksums.
This method connects to PostgreSQL and validates that complex type
data can be queried successfully, providing a basic sanity check.

=cut

sub _perform_basic_complex_type_validation
{
	my $self = shift;
	
	$self->logit("Performing basic complex type validation...\n", 1);
	...
	my $total_tables_checked = 0;
	my $total_rows_checked = 0;
	my $validation_errors = 0;
	
	# Get list of tables that potentially have complex types
	my $tables_sql = "SELECT table_name FROM information_schema.tables WHERE table_schema = ? ORDER BY table_name";
	my $schema_name = $self->{pg_schema} || $self->{schema} || 'public';
	
	my $sth = $self->{dbhdest}->prepare($tables_sql);
	if (!$sth) {
		$self->logit("ERROR: Could not prepare table list query: " . $self->{dbhdest}->errstr . "\n", 1);
		return 0;
	}
	...
	$self->logit("Found " . scalar(@tables) . " tables to validate\n", 1);
	
	# Check each table for complex type data
	foreach my $table_name (@tables) {
		$self->logit("Checking table: $table_name\n", 2) if ($self->{debug});
		
		# Get column information to identify potential complex type columns
		my $columns_sql = "SELECT column_name, data_type FROM information_schema.columns WHERE table_schema = ? AND table_name = ?";
		my $col_sth = $self->{dbhdest}->prepare($columns_sql);
		
		if (!$col_sth) {
			$self->logit("WARNING: Could not get columns for table $table_name\n", 1);
			$validation_errors++;
			next;
		}
		
		$col_sth->execute($schema_name, $table_name);
		my @complex_columns;
		
		while (my ($col_name, $data_type) = $col_sth->fetchrow_array()) {
			# Identify potential complex type columns
			if ($data_type =~ /\[\]$/ || $data_type eq 'USER-DEFINED' || $data_type =~ /^_/) {
				push @complex_columns, $col_name;
			}
		}
		$col_sth->finish();
		
		if (@complex_columns) {
			$self->logit("  Found potential complex type columns in $table_name: " . join(', ', @complex_columns) . "\n", 1);
			
			# Try to query the table to ensure data is accessible
			my $schema_prefix = '';
			if ($self->{pg_schema}) {
				$schema_prefix = $self->quote_object_name($self->{pg_schema}) . '.';
			} elsif ($self->{schema} && $self->{schema} ne 'public') {
			...
			my $test_sql = "SELECT COUNT(*) FROM " . $schema_prefix . $self->quote_object_name($table_name);
			my $count_sth = $self->{dbhdest}->prepare($test_sql);
			
			if ($count_sth && $count_sth->execute()) {
				my ($row_count) = $count_sth->fetchrow_array();
				$total_rows_checked += $row_count;
				$self->logit("  ✓ $table_name: $row_count rows accessible\n", 1);
				$count_sth->finish();
			} else {
				$self->logit("  ✗ $table_name: Query failed - " . ($self->{dbhdest}->errstr || 'Unknown error') . "\n", 1);
				$validation_errors++;
			}
	...
	# Summary
	$self->logit("\n" . "="x50 . "\n", 1);
	$self->logit("BASIC COMPLEX TYPE VALIDATION SUMMARY\n", 1);
	$self->logit("="x50 . "\n", 1);
	$self->logit("Tables with complex types: $total_tables_checked\n", 1);
	$self->logit("Total rows accessible: $total_rows_checked\n", 1);
	$self->logit("Validation errors: $validation_errors\n", 1);
	
	if ($validation_errors == 0) {
		$self->logit("✓ BASIC VALIDATION PASSED - All complex type tables are accessible\n", 1);
		$self->logit("  For comprehensive validation with data integrity checks,\n", 1);
		$self->logit("  run data export with COMPLEX_CHECKSUM_VALIDATION=1 first.\n", 1);
	} else {
		$self->logit("✗ VALIDATION ISSUES DETECTED - Some tables may have problems\n", 1);
	}
